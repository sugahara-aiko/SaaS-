...
const MASTER_DATA = [
  ['Slack', 'プロ', '01_ユーザー課金', 'コミュニケーション', '契約中', '人数課金', 1050, 'JPY', 28, '',
    '月次', '', true,
    '1. 組織外の人が混じっていないか\n2. 退職者が残っていないか(非アクティブ)\n3. 不要なシングルチャンネルゲストの削除',
    '課金対象28(管理者2+メンバー22+マルチchゲスト4)/ シングルchゲスト7は無料のため数量に含めない',
    'https://japanbitcoinindustry.slack.com/account/workspace-settings', '自動化可(users.list API)'],

  ['Notion', 'プラス', '01_ユーザー課金', '情報管理', '契約中', '人数課金', 2000, 'JPY', 24, '',
  ...
  ['Claude', 'Pro', '01_ユーザー課金', 'AIツール', '契約中', '人数課金', 20, 'USD', 5, '',
    '月次', '', true,
  ...
  ['Claude', 'Max 20x', '01_ユーザー課金', 'AIツール', '契約中', '人数課金', 200, 'USD', 9, '',
    '月次', '', true,
    '各自のアカウントで管理(解約時には解約画面を受領)\n∟管理台帳と照合',
    '個別付与(詳細は別シート)', '', '自動化不可(個人課金)'],

  ['ChatGPT', 'Pro x20', '01_ユーザー課金', 'AIツール', '契約中', '人数課金', 30000, 'JPY', 2, '',
    '月次', '', true,
    '各自のアカウントで管理(解約時には解約画面を受領)\n∟管理台帳と照合',
    '個別付与(詳細は別シート)', '', '自動化不可(個人課金)'],

  ['ChatGPT', 'Pro x5', '01_ユーザー課金', 'AIツール', '契約中', '人数課金', 16800, 'JPY', 3, '',
    '月次', '', true,
    '各自のアカウントで管理(解約時には解約画面を受領)\n∟管理台帳と照合',
    '個別付与(詳細は別シート)', '', '自動化不可(個人課金)'],

  ['ChatGPT', 'Plus', '01_ユーザー課金', 'AIツール', '契約中', '人数課金', 3000, 'JPY', 4, '',
    '月次', '', true,
    '各自のアカウントで管理(解約時には解約画面を受領)\n∟管理台帳と照合',
    '個別付与(詳細は別シート)', '', '自動化不可(個人課金)'],
  ...
  ['バクラク勤怠', 'スタンダード', '03_事務・固定費', '労務管理', '契約中', '人数課金', 125, 'JPY', 16, '',
    '年次', '', false, '', '年額¥1,500/人の月額換算(¥125/人)', 'https://attendance.layerx.jp/', ''],
...
const MASTER_FLAGS = [
  ['Vercel', 'Pro', 9, '有料シート数は不明と確認済み(2026-08)。判明したら入力'],
  ['マネフォ給与', '基本料金+300円/人(6名〜)', 7, '基本料金は不明と確認済み(2026-08)。従量単価のみ計上、判明したら修正'],
  ['マネフォ経費', '', 7, '料金は不明と確認済み(2026-08)。判明したら入力'],
  ['マネフォPay', '', 7, '料金は不明と確認済み(2026-08)。判明したら入力'],
  ['マネフォクラウド', '', 7, '料金は不明と確認済み(2026-08)。判明したら入力']
];
...
// Slackは課金区分ごとに行を分けて数値で記録する(前月比の自動計算を効かせるため)。
// ツール名の表記は毎月同一にすること(前月値はツール名の完全一致で検索される)。
const LOG_SEED = [
  ['2026-07', 'Slack(管理者)', 2, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'Slack(メンバー)', 22, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'Slack(マルチchゲスト)', 4, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'Slack(シングルchゲスト)', 7, '', '', 'OK', '無料枠(課金対象外)', '', d_(2026, 8, 4)],
  ['2026-07', 'Notion', '', '', '', '未確認', '', '', ''],
  ...
  ['2026-07', 'Claude(Max 20x)', 9, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'Claude(Max 5x)', 4, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'Claude(Pro)', 5, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'ChatGPT(Pro x20)', 2, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'ChatGPT(Pro x5)', 3, '', '', 'OK', '', '', d_(2026, 8, 4)],
  ['2026-07', 'ChatGPT(Plus)', 4, '', '', 'OK', '', '', d_(2026, 8, 4)],
